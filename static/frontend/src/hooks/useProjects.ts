...
export function useProjects() {
  ...
  const [permissions, setPermissions] = useState<UserPermissions>({ canManageColumns: false });
  ...
  const addColumn = async (label: string, type: ColumnType): Promise<boolean> => {
    ...
    try {
      const updatedCols = (await invoke('addColumn', { id, label, type })) as ColumnDef[];
  ...
  const deleteColumn = async (columnId: string, label: string): Promise<boolean> => {
    try {
      const updatedCols = (await invoke('deleteColumn', { columnId })) as ColumnDef[];
      setColumns(updatedCols);
  ...
  const updatePropertyValue = async (projectId: string, key: string, value: any) => {
    setProjects((prev) =>
      prev.map((p) =>
        p.id === projectId
